...
export function init($outerPlaceholder, focus, baseUrl, updateMathImg = ($img, latex) => {
    ...
    function onKeyDown(e) {
        if ($('.rich-text-editor-overlay').is(':visible'))
            return;
        else if (u.isUndo(e))
            undoMath();
        else if (u.isRedo(e))
            redoMath();
        else if (u.isCtrlKey(e, keyCodes.ENTER) || u.isKey(e, keyCodes.ESC))
            closeMathEditor(true);
    }
    return {
        insertNewEquation,
    ...
    function updateUndoRedoStacks() {
        ...
        if (undoRedo >= undoRedoCodes.NOCHANGE && u.last(undoStack) !== latex) {
            if (undoRedo === undoRedoCodes.NOCHANGE)
                redoStack = [];
    ...
    function onMqEdit(e) {
        ...
        mqEditTimeout = setTimeout(() => {
            if (focus.latexField || undoRedo !== undoRedoCodes.NOCHANGE)
                return;
